...
router.post('/', upload.single('capaPedido'), async (req, res) => {
  ...
  try {
    ...
    // COPIAR vendedorNome e vendedorLoja do pedidoData se vierem preenchidos do frontend
    if (!vendedorInfo.nome && (pedidoData.vendedorNome || pedidoData.nomeVendedor)) {
      vendedorInfo.nome = pedidoData.vendedorNome || pedidoData.nomeVendedor;
    }
    if (!vendedorInfo.loja && (pedidoData.vendedorLoja || pedidoData.nomeLoja)) {
      vendedorInfo.loja = pedidoData.vendedorLoja || pedidoData.nomeLoja;
    }
    ...
    // Prazo de confirmação: 2 dias após a data do pedido
    const dataLimiteConfirmacao = new Date(dataAtual);
    dataLimiteConfirmacao.setDate(dataAtual.getDate() + 2);
...
router.get('/confirmacao/:token', async (req, res) => {
  try {
    ...
    // Adicionar .populate('cliente vendedor') para garantir acesso ao nome do vendedor
    const pedido = await Pedido.findOne({ confirmacaoClienteToken: token })
      .populate('cliente')
      .populate('vendedor', 'name loja');
...
// Rota PATCH /:id - Atualizar parcialmente um pedido
router.patch('/:id', async (req, res) => {
  try {
    ...
    // Atualizar o pedido com os campos fornecidos
...
// Nova rota para upload de imagem da etapa de produção
router.patch('/:id/upload-producao', upload.single('imagemProducao'), async (req, res) => {
  try {
    const { id } = req.params;
    const { etapa } = req.body; // etapa de produção (ex: 'costura', 'corte', etc)
    
    console.log(`[BACKEND LOG] Recebido PATCH /pedidos/${id}/upload-producao para etapa: ${etapa}`);
    console.log(`[BACKEND LOG] Arquivo imagem:`, req.file);
    ...
    // Garantir que o diretório para imagens de produção exista
    const PRODUCAO_DIR = path.join(UPLOADS_DIR, 'producao');
    if (!fs.existsSync(PRODUCAO_DIR)) {
      fs.mkdirSync(PRODUCAO_DIR, { recursive: true });
    }

    // Mover o arquivo para o diretório de produção
    const imagePath = `/uploads/producao/${req.file.filename}`;
    
    // Criar ou atualizar o objeto de imagens de produção
    const imagensProducao = pedidoExistente.imagensProducao || {};
    imagensProducao[etapa] = imagePath;
    ...
    const pedidoAtualizado = await Pedido.findByIdAndUpdate(
      ...
      { $set: { 
          imagensProducao, 
          [`imagemEtapa${etapa.charAt(0).toUpperCase() + etapa.slice(1)}`]: imagePath // para compatibilidade
        } 
      },
      { new: true }
    );

    console.log(`[BACKEND LOG] Imagem de produção (${etapa}) salva com sucesso:`, imagePath);
    res.json({
      message: `Imagem de ${etapa} atualizada com sucesso`,
      imagem: imagePath,
      pedido: pedidoAtualizado
    });
  } catch (err) {
    console.error(`[BACKEND LOG] Erro ao salvar imagem de produção:`, err);
    res.status(500).json({ message: 'Erro ao salvar imagem de produção' });
  }
});
