...
export default function StepPagamento({ onNext, onBack, data }) {
  ...
  const calculatedSubtotalAcrescimos = itens.reduce((totalAcrescimos, item) => {
    ...
    const aplicacoesDoItem = aplicacoesPorItem[item.id];
    if (aplicacoesDoItem && aplicacoesDoItem.length) {
      valorUnitPersonalizacao = aplicacoesDoItem
    ...
    Object.entries(item.tamanhos || {}).forEach(([tam, qtdTamanho]) => {
      if (Number(qtdTamanho) > 0) {
        ...
        if (acrescUnitario > 0) {
          acrescimosItem += acrescUnitario * Number(qtdTamanho);
        }
      }
    });

    return totalAcrescimos + acrescimosItem;
  }, 0);

  const subtotalGeral = subtotalCostura + subtotalPersonalizacao + calculatedSubtotalAcrescimos;
    ...
    <Form onSubmit={handleSubmit} autoComplete="off">
      <div style={{ background: '#f8f9fa', borderRadius: 10, padding: 16, marginBottom: 16, color: '#15616f', fontWeight: 500, fontSize: 17 }}>
        ...
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #e0e0e0' }}>
          ...
          <div style={{ color: '#f39c12', fontWeight: 700 }}>Subtotal Acréscimos: R$ {calculatedSubtotalAcrescimos.toFixed(2)}</div>
