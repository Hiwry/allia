...
export default function PersonalizacaoEmborrachado({ aplicacoes, setAplicacoes }) {
  ...
  // Desconto: da 3ª aplicação em diante, todas as de menor valor ficam com 50% de desconto
  const sortedAplics = [...aplicacoes].sort((a, b) => b.valor - a.valor);
  let descontoAplics = sortedAplics.map((a, idx) => ({ ...a, valorFinal: idx >= 2 ? a.valor * 0.5 : a.valor }));
  ...
  return (
    <div>
      ...
      {descontoAplics.length > 0 && descontoAplics.map((a, idx) => (
        <Card key={idx} style={{ background: idx >= 2 ? '#e7f7f7' : '#f8f9fa' }}>
          <b>Aplicação {idx + 1}</b><br />
          Tamanho: {a.tamanho} | Valor: R$ {a.valorFinal.toFixed(2)} {idx >= 2 ? '(50% desconto)' : ''}<br />
          Local: {a.local}<br />
